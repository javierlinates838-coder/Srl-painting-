...
export function SectionHead({
  ...
  return (
    <div className={[centered ? "mx-auto max-w-2xl text-center" : "max-w-2xl", className].filter(Boolean).join(" ")}>
      <div className={["accent-rule", centered ? "mx-auto" : "", light ? "bg-gold" : ""].filter(Boolean).join(" ")} />
      ...
      {description ? (
        <div className={["mt-4 text-pretty", light ? "text-[1.05rem] leading-relaxed text-zinc-400" : "body-lg"].join(" ")}>
          {description}
        </div>
      ) : null}
    </div>
