        ...
        <div className="mt-14 grid gap-8 lg:grid-cols-[300px_1fr]">
          <div className="flex flex-row gap-2 overflow-x-auto pb-2 lg:flex-col lg:overflow-visible lg:pb-0">
            {services.map((s, i) => (
              ...
              >
                <span className={`icon-box h-11 w-11 shrink-0 ${i === active ? "!bg-white/15 !text-white !shadow-none" : ""}`}>
                  {icons[s.id]}
                </span>
                <div>
            ...
            <div className="relative">
              ...
              <ul className="mt-8 grid gap-3 sm:grid-cols-2">
                {current.details.map((d) => (
                  <li key={d} className="flex gap-3 rounded-xl bg-white p-3.5 text-[13px] leading-relaxed text-zinc-700 shadow-sm">
